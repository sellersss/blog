...
const NavBar = () => {
  ...
  return (
    <motion.div className='flex'>
              ...
              <Link passHref href={link.to} key={link.id} scroll={false}>
                <li
                  className={`${activeMenu === link.to ? 'bg-gray-200 dark:bg-gray-700' : ''
                    } hover:bg-gray-200 dark:hover:bg-gray-700 cursor-pointer rounded-none block py-1 px-2 nav`}
                >
      ...
      {/* Mobile Phone Menu */}
      <div className='md:hidden mr-2 block border-none'>
        <button
          type='button' aria-label='Menu'
          onClick={() => setShowMenu((showMenu) => !showMenu)}
          className='bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-700 cursor-pointer rounded-none block p-2 -mr-3 md:pb-3'
        >
                ...
                (link) =>
                  link.show && (
                    <Link passHref key={link.id} href={link.to} scroll={false}>
                      <button
                        onClick={() => setShowMenu((showMenu) => !showMenu)}
                        className='text-left hover:bg-gray-100 dark:hover:bg-gray-600 font-light block justify-between w-full px-6 py-4 leading-5'
                      >
                        {link.icon}
